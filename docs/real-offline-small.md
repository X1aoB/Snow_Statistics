...
本入口将既有 `real_lab` 的固定离线阶段串起来，默认使用 `real-small-1920`：control **2048 MiB**、compute **1920 MiB**、analysis **768 MiB**，三台合计 **4736 MiB**。另有显式的 `real-small-1792` 配置，只把 compute 改为 **1792 MiB**，合计 **4608 MiB**；已完成实际56条golden计算及独立收尾恢复，原失败控制器回执仍保留。入口按阶段执行，没有 `all`，不启动实时引擎、Hive、Iceberg或治理服务，也不切换统计来源。

**15:50 UTC更新：**本轮实际Linux故障探针通过，1792的Spark/YARN整数结果与独立基准一致；Docker返回的小写缺失容器消息使原控制器在收尾时失败，独立恢复已确认原会话和临时driver均消失，随后三台VM软关。当前在准备公共收尾兼容修复，不能把独立恢复写成原控制器正常成功。详见[本轮计算与恢复](evidence/offline-1792-recovery-20260919.json)；下表15:23以前的记录保留其时点范围。

**当前操作状态（2026-09-19）：**用户已恢复任务，按具体节点、源码和资源门禁分阶段执行。早先“一律暂停三机、禁止继续开机”的安排是宿主异常中断后的历史停点，不再代表当前授权。宿主 `0xF7` 的原因仍未确定，代码修复、一次启动或软关成功都不表示蓝屏根因已解决。15:18–15:23 UTC三台已按独立窗口完成已签名、Linux CI通过的 `d17caae` 源码更新和软关读回；随后已进入新的1792联调窗口。[本轮安装证据](evidence/postcrash-source-install-d17-20260919.json)只证明这些安装窗口，不代表之后VM持续关闭或完整引擎验收已通过。

| 范围 | 当前可确认结果 | 下一步验证边界 |
|---|---|---|
| `real-small-1920` 历史小样本 | 9月19日的56条合成golden已在YARN通过，较早真实前缀也已完成HDFS双副本与原Kafka确认 | 原配置和输入范围保留，不能替代1792或新入口验收 |
| `real-small-1792` 新入口 | 9月19日15:02 UTC曾实际完成三机启动；后续安装私有工具前，监测把Docker挂载列表顺序变化误判成容器替换，已按精确归属收尾 | 未运行新golden、Hive或Iceberg；完整流程仍待实际验收 |
| 挂载顺序修复 `d17caae` | 只对完整挂载记录排序后计算身份摘要，保留所有字段及原校验；Linux CI 35450927709 / 35450925871成功 | 三台实际源码安装已通过；新golden/Hive/Iceberg仍待独立实际回执 |
| 正式数据后续 | 新质量前缀已完成Kafka/Flink/Doris与线上SQLite同截止对账 | 该新前缀的capture、HDFS落地与ACK另行完成；自然闭日报表等待真实日期闭合 |

当前操作路线是：已完成逐台同版本安装与软关读回，正在用明确的1792配置进入三机窗口；先执行隔离合成故障探针和小样本golden，之后才进行新的Hive/Iceberg联调。真实增量的capture/ACK使用独立storage窗口，日指标与行为计算要求合法闭日和原生命周期许可。每次转换都遵守本页的资源、来源和精确收尾规则，不把源码安装、容器健康或外层命令退出0当作下一阶段已通过。该恢复顺序没有扩大内存、磁盘限额，也没有延长真实数据期限。

对应实现：[Windows/节点模块](../src/snow_statistics/real_offline_small.py)、[CLI](../tools/real_offline_small.py)、[合成故障测试](../tests/test_real_offline_small.py)。冻结的 `real_lab.py`、writer、恢复账本、生命周期及模型代码均继续使用原实现。更完整的已完成/待完成边界见[交付清单](completion-checklist.md)，异常中断事实见[宿主中断证据](evidence/host-interruption-20260919.json)。

## 历史入口验证（不代替当前版本验收）

9月19日的 `46efeceb` 已通过Linux CI35438865825（含实际`SIGHUP`进程组测试），Windows精确进程树终止也已在本地测试。首次实际VM入口尝试通过正式paused/stopped准入并启动三台VM，但在analysis DataNode的额外`/data`匿名父卷检查处失败；原失败回执保留，不能记为完整启动成功。操作者核对实际容器归属后，精确停止该DataNode并软关三台VM，没有删除卷，也没有继续后续计算。最小兼容修复 `22ef47ae` 的完整本地745项测试和Linux CI35439860162、35439858210通过。此前同配置的手工编排、合成golden及正式ODS结果仍是各自的独立证据。

第二轮 `22ef47ae` 通过实际匿名卷归属校验，但节点子任务成功后，心跳守护线程在解释器退出时占用标准输入，导致 `_enter_buffered_busy` 和非零 SSH 退出。本轮仍是失败；已精确停止所属服务并软关全部VM，未提交计算。后续修复采用主循环有界管道读取并处理正常退出时的断管竞态，Linux CI及逐台标准输入保持打开的零退出回归另有记录。关闭管道不表示任务成功，仍要求实际零退出及结构化结果；完整三机和引擎成绩按版本单独记载，见[实施状态](status.md)。
...
首次实际检查中，锁定Hadoop镜像的容器出现了`/data`匿名父卷，数据目录另由明确命名卷挂到`/data/name`、`/data/dn`或`/data/yarn`。本入口不忽略所有匿名卷：只有实际镜像`Config.Volumes`恰好声明`/data`，且具体64位十六进制卷名、创建时间、本地驱动、实际挂载与含停止容器在内的独占引用都核验一致，才将该父卷纳入容器身份摘要。Docker可能在相邻inspect中改变挂载列表顺序；完整挂载记录经过规范排序后才计算身份摘要，不改变或省略记录中的字段。额外挂载、绑定目录冒充、卷复用或已观测卷身份变化仍拒绝。不删除原匿名卷，也不把合成的卷元数据测试当成真实后端读回。
...
Docker驱动已经自动移除时，仅接受精确CID或固定Hive/湖仓驱动名对应的已知缺失对象消息、有界stdout/stderr及退出码1；消息前缀大小写不影响判定。连接失败、权限错误、其他对象或额外输出仍保留失败。阶段容器的完整挂载字段排序后计算身份摘要，但不迁移旧lease中的摘要：部署前必须完成原阶段的恢复并确认没有未结束的owner，不能改历史回执使其通过。
