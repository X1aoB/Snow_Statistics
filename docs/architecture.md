...
时间统一 UTC 存储；业务日期 Asia/Hong_Kong。接收过去 7 天至未来 5 分钟的事件，避免任意历史写入破坏已过期去重状态。原始保留按接收时间计算 7 天，标识辅助/去重保留 30 天，日聚合 90 天。2 GiB 独立文件系统提供硬边界，应用保守预留空间并明确拒收；不因消费者离线删除已确认保留期数据。7 天是待容量验证的目标，拒收会形成采集缺口，汇总完整性只承诺已接收事件。
...
主转化链路：点击 → 到达 → 选择角色 → 前端请求关联 → 服务端成功。成功必须在入口点击后 30 分钟内，归于最近有效点击，每个点击最多一次。缺少前端同意或任一关联事件的成功不做猜测归因。两个应用不合并长期身份。
