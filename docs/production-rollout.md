...
本节覆盖下方带日期的历史状态。[宣传录制移交](promotion-handoff.md)已经发送，真实数仓实测继续独立执行；尚无真实生成样例的质量投递单列待观察。私有发布回执位于 `runtime/consent-candidate/` 与 `runtime/production-candidate/`；公开材料只引用哈希、版本和检查范围。
