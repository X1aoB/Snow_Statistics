...
def main():
    ...
    directory = ROOT / "runtime/scale"
    receipt = directory / (args.attempt + "-monitor.json")
    if receipt.exists():
    ...
    script = directory / (args.attempt + "-submit.sh")
    script.write_text(f"set -euo pipefail\ncd /home/snow/Snow_Statistics\nbash tools/run_scale_batch.sh {args.events} {args.attempt}\n", encoding="utf-8", newline="\n")
    ...
    with (directory / (args.attempt + "-submit.log")).open("wb") as log:
